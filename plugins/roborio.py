...
def ProcessBrownedOut(robotTelemetry: pd.DataFrame):
...
def ProcessCanUtilization(robotTelemetry: pd.DataFrame):
...
def ProcessCanOffCount(robotTelemetry: pd.DataFrame):
...
def ProcessCanRxErrorCount(robotTelemetry: pd.DataFrame):
    ...
    '''
    canRxErrCount = robotTelemetry[robotTelemetry['Key'] == 'RoboRio CAN Rx Error Count']
...
def ProcessCanTxErrorCount(robotTelemetry: pd.DataFrame):
    ...
    '''
    canTxErrCount = robotTelemetry[robotTelemetry['Key'] == 'RoboRio CAN Tx Error Count']
...
def ProcessCanTxFullCount(robotTelemetry: pd.DataFrame):
    ...
    '''
    canTxFullCount = robotTelemetry[robotTelemetry['Key'] == 'RoboRio CAN Tx Full Count']
...
def ProcessStaleDsData(robotTelemetry: pd.DataFrame):
...
def ProcessImuYawAngleNorm(robotTelemetry: pd.DataFrame):
...
def ProcessImuYawAngleDrift(robotTelemetry: pd.DataFrame):
    fmsMode = robotTelemetry[robotTelemetry['Key'] == 'FMS Mode']
    if fmsMode.empty:
        return -1, 'metric_not_implemented'
    startTime = fmsMode[fmsMode['Value'] == 'Disabled']['Timestamp'].min()
    ...
    linearModel = np.polyfit(imuYawAngle['Timestamp'], imuYawAngle['IMU Yaw Angle (deg)'], 1)
